...
export interface MyCollectionResponse {
  ...
  limit: number;
  last_page: number;
  /** 是否还有下一页（在 total/last_page 不准确时仍可下拉加载） */
  has_more: boolean;
}

function readOptionalHasMoreFlag(value: unknown): boolean | null {
  ...
  if (typeof value === 'string') {
    ...
    if (next === '1' || next === 'true' || next === 'yes') {
      return true;
    }
    if (next === '0' || next === 'false' || next === 'no' || next === '') {
      return false;
    }
  }
  return null;
...
/**
 * 权益节点等分类下，接口常返回 total=0 或 last_page=1，但实际仍有多页数据。
 * 在信不过分页元数据时，用「满页 + 已加载条数 vs total」兜底判断是否继续请求。
 */
function computeMyCollectionHasMore(params: {
  listLength: number;
  page: number;
  lastPage: number;
  limit: number;
  ...
  const { listLength, page, lastPage, limit, total } = params;
  if (listLength <= 0) {
    return false;
  }
  if (page < lastPage) {
  ...
  const loadedCount = (page - 1) * limit + listLength;
  if (total <= 0) {
    return true;
  }
  ...
  myCollection(
    ...
    return http
      ...
      .then((response): MyCollectionResponse => {
        ...
        const list = Array.isArray(response.list) ? response.list.map(normalizeMyCollectionItem) : [];

        const serverHasMore = readOptionalHasMoreFlag(response.has_more ?? response.hasMore);
        const hasMore =
          serverHasMore !== null
            ? serverHasMore
            : computeMyCollectionHasMore({
