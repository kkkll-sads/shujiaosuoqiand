...
import { Clock3, Loader2, PackageCheck, Slash, RefreshCw } from 'lucide-react';
...
export const AfterSalesPage = () => {
  ...
  const [accOrders, setAccOrders] = useState<ShopOrderListItem[]>([]);
  ...
  useEffect(function () {
    ...
    setAccOrders(request.data.list);
    setCurrentPage(1);
    setHasMore(request.data.list.length >= PAGE_SIZE && request.data.list.length < request.data.total);
  }, [request.data]);

  /** 筛选出含有售后信息的订单 */
  const afterSaleOrders = useMemo(
    () =>
      accOrders.filter((order) => {
  ...
  const loadMore = useCallback(async function () {
    ...
    try {
      ...
      var result = await shopOrderApi.myOrders({ page: nextPage, limit: PAGE_SIZE });
      setAccOrders(function (prev) {
  ...
  return (
    <div className="flex h-full flex-1 flex-col overflow-hidden bg-bg-base">
      ...
      <div ref={scrollContainerRef} className="flex-1 overflow-y-auto px-4 pb-6">
