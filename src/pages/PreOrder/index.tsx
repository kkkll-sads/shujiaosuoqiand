...
export const PreOrderPage = () => {
  ...
  const [useMixedPayment, setUseMixedPayment] = useState<0 | 1 | undefined>(undefined);
  ...
  /** 点击「确认预约」→ 调用预约预览 API（首次不传 use_mixed_payment，由服务端决定默认值） */
  const handleSubmit = useCallback(async () => {
    if (!canSubmit || !selectedZone) return;
    setPreviewLoading(true);
    setPreviewError('');
    setUseMixedPayment(undefined);
    try {
  ...
  return (
    <div className="flex-1 flex flex-col bg-bg-base relative overflow-hidden">
      ...
      <PreviewSheet
        ...
        onMixedPaymentChange={setUseMixedPayment}
      />
    </div>
  );
};
