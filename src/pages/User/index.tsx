...
import { activityPopupApi, type ActivityPopupItem } from '../../api/modules/activityPopup';
...
import {
  ActivityPopupModal,
  isActivityPopupDismissed,
  dismissActivityPopup,
} from '../../components/biz/ActivityPopupModal';
...
export const UserPage = () => {
  ...
  const [activityPopupVisible, setActivityPopupVisible] = useState(false);
  const [activityPopupList, setActivityPopupList] = useState<ActivityPopupItem[]>([]);
  const [activityPopupIndex, setActivityPopupIndex] = useState(0);

  const currentActivityPopup = activityPopupList[activityPopupIndex] ?? null;
  const hasNextActivity = activityPopupIndex < activityPopupList.length - 1;

  useEffect(() => {
    const ac = new AbortController();
    const timer = window.setTimeout(async () => {
      try {
        const list = await activityPopupApi.getPopupList(ac.signal);
        const toShow = list.filter((item) => !isActivityPopupDismissed(item));
        if (toShow.length === 0) return;
        setActivityPopupList(toShow);
        setActivityPopupIndex(0);
        const delay = Math.max(0, toShow[0].popupDelay);
        if (delay > 0) {
          window.setTimeout(() => setActivityPopupVisible(true), delay);
        } else {
          setActivityPopupVisible(true);
        }
      } catch {
        /* 静默忽略 */
      }
  ...
  const renderHeader = () => {
    if (isHeaderLoading) {
      return (
        <div className="relative bg-gradient-to-b from-red-50 to-bg-base px-4 pt-4 pt-safe pb-6">
          <div className="absolute top-4 right-4 flex space-x-4 pt-safe">
    ...
    return (
      <div className="relative z-10 pt-4 pt-safe pb-2">
        <ProfileHeader
  ...
  return (
    <div className="relative flex flex-1 flex-col overflow-hidden bg-bg-base">
      ...
      {activityPopupVisible && currentActivityPopup && (
        <ActivityPopupModal
          item={currentActivityPopup}
          isOpen={activityPopupVisible}
          hasNext={hasNextActivity}
          onClose={() => {
            dismissActivityPopup(currentActivityPopup);
            if (hasNextActivity) {
              setActivityPopupIndex((i) => i + 1);
            } else {
              setActivityPopupVisible(false);
            }
          }}
          onNext={() => {
            dismissActivityPopup(currentActivityPopup);
