...
export const PaymentResultPage = () => {
  ...
  const renderHeader = () => (
    <div className="bg-white dark:bg-gray-900 z-40 relative shrink-0 border-b border-border-light">
      ...
      <div className="h-12 flex items-center justify-between px-3 pt-safe">
        ...
        <h1 className="text-[16px] font-bold text-text-main text-center w-1/3">支付结果</h1>
  ...
  const renderSuccess = () => (
    <div className="flex flex-col items-center pt-12 px-6">
      <div className="w-20 h-20 rounded-full bg-[#00C853]/10 flex items-center justify-center mb-4">
        <CheckCircle2 size={48} className="text-[#00C853]" />
      </div>
      <h2 className="text-[20px] font-bold text-text-main mb-2">支付成功</h2>
      <div className="text-[28px] font-bold text-text-main mb-1 flex items-baseline">
        <span className="text-[18px] mr-1">¥</span>
        {orderInfo.amount.toFixed(2)}
      </div>
      <div className="flex items-center text-[13px] text-text-sub mb-10">
      ...
      <div className="w-full flex space-x-4">
        <button 
          ...
          className="flex-1 h-11 rounded-full border border-primary-start text-primary-start text-[15px] font-medium active:bg-primary-start/5"
        >
          继续逛逛
        </button>
        <button 
          ...
          className="flex-1 h-11 rounded-full bg-gradient-to-r from-primary-start to-primary-end text-white text-[15px] font-medium shadow-sm active:opacity-80"
        >
          查看订单
        </button>
      </div>
  ...
  const renderFailure = () => (
    <div className="flex flex-col items-center pt-12 px-6">
      ...
      <h2 className="text-[20px] font-bold text-text-main mb-6">支付失败</h2>
      
      <div className="w-full bg-bg-base rounded-[12px] p-4 mb-10 relative">
        <div className="text-[13px] text-text-sub leading-relaxed pr-8">
      ...
      <button 
        className="flex items-center text-[13px] text-text-sub active:opacity-70"
        onClick={() => {
  ...
  return (
    <div className="flex-1 flex flex-col bg-white dark:bg-gray-900 relative h-full overflow-hidden">
      ...
      <div className="px-4 py-2 flex space-x-2 overflow-x-auto no-scrollbar bg-bg-card border-b border-border-light text-[10px] absolute top-12 left-0 right-0 z-50 opacity-50 hover:opacity-100 transition-opacity">
        ...
        <button onClick={() => setStatus('success')} className={`px-2 py-1 rounded border ${status === 'success' ? 'bg-primary-start text-white border-primary-start' : 'border-border-light'}`}>Success</button>
        <button onClick={() => setStatus('failure')} className={`px-2 py-1 rounded border ${status === 'failure' ? 'bg-primary-start text-white border-primary-start' : 'border-border-light'}`}>Failure</button>
