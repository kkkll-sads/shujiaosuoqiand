...
export function RightsPage() {
  ...
  const growthReloadRef = useRef<(() => Promise<unknown>) | null>(null);
  ...
  const handleRefresh = useCallback(async () => {
    if (activeTab === 'apply') {
      ...
      if (growthReloadRef.current) {
        await growthReloadRef.current().catch(() => undefined);
      }
    }
        ...
        {activeTab === 'growth' ? (
          <div ref={scrollContainerRef} className="flex-1 min-h-0 overflow-y-auto">
            <div className="animate-in fade-in duration-300">
              <GrowthRightsContent onReloadRef={growthReloadRef} />
            </div>
          </div>
        ) : (
          <div
